...
def render_operations(filtered_df, df, period_type, selected_period):
    ...
    with c1:
        ...
        if not return_df.empty:
            ...
            fig_return = px.bar(return_reason_counts, x='Alasan', y='Jumlah', title=f"Alasan Retur Tertinggi ({selected_period})",
                                color='Jumlah',
                                text_auto=True, color_continuous_scale=['#E0E7FF', '#6366F1'])
    ...
    with c2:
        ...
        if not delivery_df.empty:
            ...
            fig_delivery.add_vline(x=target_q3, line_dash="dash", line_color="#A5B4FC", annotation_text=f"Target: {target_q3:.1f}")
            fig_delivery.update_traces(marker_color=['#4F46E5' if x == max_delivery else '#A5B4FC' for x in top10_lambat['delivery_days']])
    ...
    with c3:
        ...
        fig_ops = go.Figure()
        fig_ops.add_trace(go.Bar(x=monthly_ops['label'], y=monthly_ops['order_id'], name='Volume Pesanan', marker_color='#A5B4FC', yaxis='y1'))
        fig_ops.add_trace(go.Scatter(x=monthly_ops['label'], y=monthly_ops['is_returned'], name='Tingkat Retur (%)', mode='lines+markers', line=dict(color='#4F46E5', width=3), yaxis='y2'))

...
def render_operations(filtered_df, df, period_type, selected_period):
    ...
    with c1:
        ...
        if not return_df.empty:
            ...
            fig_return = px.bar(return_reason_counts, x='Alasan', y='Jumlah', title=f"Alasan Retur Tertinggi ({selected_period})",
                                color='Jumlah',
                                text_auto=True, color_continuous_scale=['#DDD6FE', '#7C3AED'])
    ...
    with c2:
        ...
        if not delivery_df.empty:
            ...
            fig_delivery.add_vline(x=target_q3, line_dash="dash", line_color="#8B5CF6", annotation_text=f"Target: {target_q3:.1f}")
            fig_delivery.update_traces(marker_color=['#4338CA' if x == max_delivery else '#818CF8' for x in top10_lambat['delivery_days']])

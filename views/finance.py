...
def render_finance(filtered_df, df, period_type, selected_period):
    ...
    with r1c1:
        ...
        fig_fin_area.add_trace(go.Scatter(x=monthly_fin['label'], y=monthly_fin['net_revenue'], fill='tozeroy', name='Pendapatan Bersih', mode='lines+markers', fillcolor='rgba(139, 92, 246, 0.6)', line=dict(color='#8B5CF6'), yaxis='y1'))
    ...
    with r1c2:
        ...
        fig_disc_cat = px.bar(disc_cat, x='discount_value', y='product_category', orientation='h',
                              ...
                              text_auto='.2s', color_continuous_scale=['#C7D2FE', '#4F46E5'])

...
def render_marketing(filtered_df, df, period_type, selected_period):
    ...
    with r1c1:
        ...
        fig_dual = go.Figure()
        fig_dual.add_trace(go.Bar(x=monthly_mkt['label'], y=monthly_mkt['final_price'], name='Pendapatan', marker_color='#2B5A96', yaxis='y1'))
        fig_dual.add_trace(go.Scatter(x=monthly_mkt['label'], y=monthly_mkt['discount_percent'], name='Diskon (%)', mode='lines+markers', line=dict(color='#0F2545', width=2), yaxis='y2'))
        ...
        if target_discount is not None and not pd.isna(target_discount) and period_type != "Semua":
            fig_dual.add_hline(y=target_discount, line_dash="dash", line_color="#0B1A30", 
    ...
    with r2c1:
        fig_retention = go.Figure(data=go.Heatmap(
            ...
            colorscale=[[0, '#EEF2FF'], [0.5, '#2B5A96'], [1, '#0F2545']],
    ...
    with r2c2:
        fig_revenue = go.Figure(data=go.Heatmap(
            ...
            colorscale=[[0, '#EEF2FF'], [0.5, '#183B68'], [1, '#0B1A30']], # Diganti dari Pink ke Indigo/Ungu
